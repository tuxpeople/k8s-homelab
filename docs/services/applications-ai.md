...
-   **Backups**: PVC `data` enthält Uploads, Logs, Bilder → via K8up Job für Namespace `ai` sichern (RPO 24 h). MongoDB/Meili laufen im Pod (kein StatefulSet), daher zusätzlich regelmässige Dumps einplanen.
-   **Monitoring / Alerts**:
...
-   **Storage**:
    -   PVC `models` 60 Gi (Model-Files) – Longhorn, unbedingt regelmässig snapshotten (RPO 12 h).
