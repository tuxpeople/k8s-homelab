...
- **Speicher**:
  ...
  - `cache` via `emptyDir`.
- **Secrets / Env**: Keine sensiblen Variablen außer `PUID/PGID/TZ`.
- **Backups**:
...
- **Zweck**: Stellt ausschließlich Ingress + `ExternalName`-Services bereit, die auf eine externe Mediabox-Installation (`*.media.tdeutsch.ch`) verweisen (Docker Stack).
